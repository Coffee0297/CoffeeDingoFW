...
static void SystemClock_Config(void)
{
  ...
  /* dingoFW: clock the bootloader from the 8 MHz HSE crystal, NOT the internal HSI RC.
   * CAN bit timing must be derived from an accurate clock. The HSI RC oscillator is only
   * ~+-1% (worse over temperature) which is not precise enough for reliable 500 kbit CAN:
   * against the crystal-based nodes on the bus it produces stuff errors and the receiver's
   * error counter pins at 255 (observed on hardware). Use the same clock tree as the
   * application: HSE 8 MHz x9 = 72 MHz SYSCLK, APB1 /2 = 36 MHz CAN clock.
   * (If the crystal is dead, CAN cannot work anyway, so there is no robustness lost vs HSI;
  ...
  /* Configure and enable the PLL: HSE/1 x9 = 72 MHz. */
  ...
  LL_RCC_SetAHBPrescaler(LL_RCC_SYSCLK_DIV_1);   /* HCLK  = 72 MHz */
  LL_RCC_SetAPB1Prescaler(LL_RCC_APB1_DIV_2);    /* PCLK1 = 36 MHz (bxCAN clock) */
  LL_RCC_SetAPB2Prescaler(LL_RCC_APB2_DIV_1);    /* PCLK2 = 72 MHz */
...
void HAL_MspInit(void)
{
  ...
  /* dingoFW: the OpenBLT demo drives PB3 as a status LED, but on the CANBoard PB3 is DO2
   * (a Profet load-driver output). Deliberately leave PB3 in its reset state (input) so the
   * bootloader never energizes a load. The LedBlink* calls in hooks.c/led.c only toggle the
   * PB3 ODR bit, which has no electrical effect while the pin is an input. */
