...
/****************************************************************************************
* dingoFW: runtime CAN configuration (base id + speed) passed by the application
****************************************************************************************/
/* The dingoPDM stores its config in external I2C FRAM, which this minimal bootloader
 * cannot read. Instead the application, when it enters the CAN bootloader, leaves its base
 * id and CAN speed (alongside the activation magic) in a fixed SRAM block at the top of
 * RAM - see RequestBootloaderCan() in boards/cortex-m4/mcu_utils.cpp. That block is outside
 * this bootloader's linked RAM (its RAM length is 16 bytes short of 128 kB), so neither the
 * bootloader's .bss nor its stack clobbers it. On a cold boot / invalid app the magic is
 * absent and the blt_conf.h fallbacks (PDM default base 0x0DE) are used. */
#define DINGO_BL_PARAM_ADDR     (0x2001FFF0UL)   /* top 16 bytes of the 128 kB SRAM        */
#define DINGO_BL_MAGIC          (0xB00710ADUL)   /* app wrote valid params before reset    */
#define DINGO_XCP_CMD_OFFSET    (12u)            /* XCP cmd id  = base + 12 (tool->bl)     */
#define DINGO_XCP_RSP_OFFSET    (13u)            /* XCP resp id = base + 13 (bl->tool)     */
...
/************************************************************************************//**
** \brief     Reads the base id and CAN speed the application left in SRAM and derives the
**            runtime baudrate and XCP message ids from them. Leaves the blt_conf.h
**            fallbacks in place if the magic is absent or the values are out of range.
** \return    none.
****************************************************************************************/
static void DingoLoadCanConfig(void)
{
  volatile const blt_int32u *param = (volatile const blt_int32u *)DINGO_BL_PARAM_ADDR;
  ...
  if (param[0] != DINGO_BL_MAGIC)
  {
    return;                                 /* cold boot / no params -> keep fallbacks */
  }
  baseId   = param[1];
  canSpeed = param[2];
  ...
  {
    dingoRxMsgId = baseId + DINGO_XCP_CMD_OFFSET;
    dingoTxMsgId = baseId + DINGO_XCP_RSP_OFFSET;
  }
...
void CanInit(void)
{
  ...
  /* dingoFW: pull the baudrate and XCP message ids from the params the app left in SRAM
   * (falls back to the blt_conf.h defaults if the magic is absent/invalid). */
  DingoLoadCanConfig();
