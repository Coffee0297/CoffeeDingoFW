...
{
  /* dingoFW: write the regular (body) block FIRST and the bootblock (which holds the vector
   * table at the application's base address) LAST. The application is judged valid by its
   * vector table (see NvmVerifyChecksumHook in the bootloader's hooks.c), so writing the
   * vectors strictly last makes "valid vectors present" imply "the whole image was
   * programmed". If a CAN update is interrupted (brown-out), the vectors are never written,
   * the app reads as invalid, and the bootloader stays active and re-flashable.
   * (OpenBLT's stock order wrote the bootblock first, leaving a tiny window in which the
   * vectors were present but the final body block was not - this reorder closes it. It is
   * also correct for the stock checksum method, since the checksum is injected into the
   * bootblock before FlashDone.) */
  ...
  /* write the boot block (vector table) LAST, so a complete vector table proves a complete
   * application image */
  if (bootBlockInfo.base_addr != FLASH_INVALID_ADDRESS)
  {
